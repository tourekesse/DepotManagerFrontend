...
function DashboardSidebarPageItem({
  ...
  const handleClick = React.useCallback(() => {
    ...
    // Fermer le drawer sur mobile quand on clique sur un lien
    if (setExpanded) {
      // Essayer de fermer sur mobile, sinon fermer sur tablette aussi
      if (isMobile) {
        setExpanded(false);
      } else if (window.innerWidth < 960) { // Fallback pour tablette
        setExpanded(false);
      }
    }
  }, [onPageItemClick, id, nestedNavigation, setExpanded, isMobile]);
