...
const BoissonApp = () => {
  ...
  const Resume = () => {
    ...
    const clientNom = clientSelectionne?.raisonsociale || "—";
    const livreurNom = livreurSelectionne ? `${livreurSelectionne.firstName} ${livreurSelectionne.lastName}` : null;
    ...
    return (
      ...
      >
        {/* Ligne discrète Client/Livreur */}
        <Typography variant="caption" color="text.secondary" sx={{ mb: 0.5 }}>
          Client : <b>{clientNom}</b>
          {livreurNom && (
            <>
              {" "}
              | Livreur : <b>{livreurNom}</b>
            </>
          )}
        </Typography>
        ...
        <Typography sx={{ fontWeight: 900, mb: 0.5 }}>Récapitulatif</Typography>

        {/* Liste simplifiée des produits - compacte */}
        <Box sx={{ display: "flex", flexDirection: "column", gap: 0.25, maxHeight: "140px", overflow: "auto" }}>
          {cart.map((item) => {
            ...
            return (
              <Box key={item.id} sx={{ display: "flex", alignItems: "center" }}>
                ...
                <Box sx={{ flex: 1, borderBottom: "1px dashed", mx: 0.75, opacity: 0.35 }} />
        ...
        {/* Bloc financier - amplifié */}
        <Box sx={{ display: "flex", flexDirection: "column", gap: 0.5 }}>
          <Box sx={{ display: "flex", alignItems: "center" }}>
            <Typography sx={{ fontWeight: 700, fontSize: "0.9rem" }}>Total brut</Typography>
          ...
          <Box sx={{ display: "flex", alignItems: "center", color: totalConsigne > 0 ? "error.main" : "inherit" }}>
            <Typography sx={{ fontWeight: 700, fontSize: "0.9rem" }}>Emballages rendus</Typography>
            <Box sx={{ flex: 1, borderBottom: "1px dotted", mx: 1, opacity: 0.4 }} />
          ...
          {typeVente === "CASH_ECHANGE" && (
            <Box sx={{ display: "flex", alignItems: "center", color: "error.main" }}>
              <Typography sx={{ fontWeight: 700, fontSize: "0.9rem" }}>Vides rendus</Typography>
              <Box sx={{ flex: 1, borderBottom: "1px dotted", mx: 1, opacity: 0.4 }} />
