...
const BoissonApp = () => {
  ...
  const ajouterAuPanier = (boisson) => {
    ...
    // Notification eBay style
    setLastAddedItem({ ...itemData, quantite: qte });
  ...
  const confirmerVente = async () => {
    ...
    try {
      ...
      // Utiliser le nouveau modal de succès eBay style
  ...
  return (
    <>
      {/* Notification d'ajout au panier style eBay */}
      <CartNotification 
      ...
      {/* Bubble du panier flottant */}
      <CartBubble 
        count={cart.length}
        total={typeVente === "VENTE_CREDIT" ? 0 : netCash}
        onClick={() => console.log("Cart clicked")} // ou naviguer vers le panier
      />
      ...
      <SuccessConfirmation
        ...
        handleWhatsAppShare={sendWhatsAppReceipt}
        handleSendReleveWhatsApp={() => console.log("Send releve")}
      />
