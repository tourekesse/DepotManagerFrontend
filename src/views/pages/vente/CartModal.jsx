          ...
          {cart.length === 0 ? (
              ...
              <>
                {cart.map((item, index) => {
                  ...
                  const consigneBouteilleUnit = Number(item.consigne_bouteille || item.consigneBouteille || 0);
                  const consigneCasierUnit = Number(item.consigne_casier || item.consigneCasier || 0);
                  const casiersPhysiques = item.casiers_physiques != null ? item.casiers_physiques : Math.ceil(quantite);
                  const nbBouteillesParCasier = Number(item.nbre_bouteilles || item.nbBouteillesParCasier || 0);
                  const totalBouteilles = Number(item.total_bouteilles || (quantite * nbBouteillesParCasier));
                  const consigneTotale = casiersPhysiques * consigneCasierUnit + totalBouteilles * consigneBouteilleUnit;

                  // Calcul du total liquide pour la ligne
                  const totalLiquideLigne = prix * quantite;
                      ...
                      <Box key={item.id || index} sx={{ p: 2, borderBottom: "1px solid #eee" }}>
                        <Box sx={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                          <Box sx={{ flex: 1 }}>
                            ...
                            {/* Modification ici : Ajout du sous-total du liquide au bout de la ligne */}

                              {/* Rendu optimisé de la ligne prix × quantité */}
                              <Typography variant="body2" color="text.secondary" sx={{ display: "flex", alignItems: "center", gap: 0.5 }}>
                                  {formatF(prix)} × {quantite} <span style={{ color: '#757575', margin: '0 4px' }}>=</span> <b style={{ color: '#1976d2', fontWeight: 700 }}>{formatF(totalLiquideLigne)}</b>
                              </Typography>

                              {consigneTotale > 0 && (
                                <Typography variant="body2" color="text.secondary" sx={{ fontSize: "0.8rem" }}>
                                  + Emballages: {casiersPhysiques} casier{casiersPhysiques > 1 ? 's' : ''} × {formatF(consigneCasierUnit)} + {Math.round(totalBouteilles)} bouteille{Math.round(totalBouteilles) > 1 ? 's' : ''} × {formatF(consigneBouteilleUnit)} = {formatF(consigneTotale)}
                                </Typography>
                            )}
                          </Box>
                          <Box sx={{ display: "flex", alignItems: "center", gap: 1 }}>
                            <IconButton
                                size="small"
                                onClick={() => modifierQuantitePanier(item, -1)}
